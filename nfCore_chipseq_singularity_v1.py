...
if dataframe[["FASTQ.R2"]].isnull().values.all():
    ...
    dataframe["fastq_1"] = seqpath+"/"+dataframe['Sequencing run ID']+"/FastQ/"+dataframe['FASTQ.R1']
    dataframe = dataframe[['sample', 'fastq_1']]
    dataframe = dataframe[~dataframe.isin([np.nan]).any(axis=1)]
    dataframe = dataframe.assign(fastq_2 = "")
    dataframe = dataframe.assign(replicate = dataframe.reset_index().index + 1 )
else:
    dataframe["sample"] = dataframe["Experiment ID"] +"_"+dataframe["Sample ID"]
    dataframe["fastq_1"] = seqpath+"/"+dataframe['Sequencing run ID']+"/FastQ/"+dataframe['FASTQ.R1']
    dataframe["fastq_2"] = seqpath+"/"+dataframe['Sequencing run ID']+"/FastQ/"+dataframe['FASTQ.R2']
...
yaml_file = "/project/imoskowitz/shared/software/assets/nfcore_ChIPseq_params_UCSCmm10.yaml"
...
shell = "nextflow run \\\n"+
"/project/imoskowitz/shared/software/nf-core-chipseq_2.0.0/2.0.0 -work-dir "+'/scratch/midway3/'+os.getlogin()+"/working_"+seqrunid+ " \\\n"+
"-profile singularity -params-file "+args.out+"/"+seqrunid+"/chipseq_params.yaml -c /project/imoskowitz/shared/software/assets/test3.config \n")
